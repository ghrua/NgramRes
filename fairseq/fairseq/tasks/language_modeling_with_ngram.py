...
import sys
# sys.path.append("/mnt/task_wrapper/user_output/artifacts/KNQuery/")
...
@dataclass
class LanguageModelingWithNgramConfig(FairseqDataclass):
...
@register_task("language_modeling_with_ngram", dataclass=LanguageModelingWithNgramConfig)
class LanguageModelingWithNgramTask(LegacyFairseqTask):
    """
    ...
    def __init__(self, args, dictionary, output_dictionary=None, targets=None, ngram_generation_model=None, ngram_warmup_updates=0, ngram_dist_size=100, eval_ngram_min_ctxlen=2, eval_ngram_min_accum_prob=0.9, input_with_ctxlen=False, ngram_ctxlen_p="", data2clst_map_path="", train_with_limited_ctxlen=False, ngram_min_dist_prob=1.0, weighted_ctxlen=False):
        super().__init__(args)
        self.dictionary = dictionary
        if data2clst_map_path:
            data2clst_map_path_list = data2clst_map_path.split(";")
            ...
            self.valid_data2clst_map = None
        self.train_with_limited_ctxlen = train_with_limited_ctxlen
        ...
        self.ngram_warmup_updates = ngram_warmup_updates
        self.ngram_dist_size = ngram_dist_size
        self.eval_ngram_min_ctxlen = eval_ngram_min_ctxlen
        self.eval_ngram_min_accum_prob = eval_ngram_min_accum_prob
        self.input_with_ctxlen = input_with_ctxlen
        self.ngram_context_len = self.ngram_generation_model.context_len
        self.ngram_min_dist_prob = ngram_min_dist_prob
        self.weighted_ctxlen = weighted_ctxlen
        if ngram_ctxlen_p:
            self.ngram_ctxlen_p = [0.0] + [float(it) for it in ngram_ctxlen_p.strip().split(";")]
        else:
    ...
    def setup_task(cls, args, **kwargs):
        ...
        sys.path.append(args.ngram_module_path)
        import c_fast_model
        ngram_generation_model = c_fast_model.CMultiFastGenerationModel.from_cache(
            args.ngram_generation_model_cache, dictionary.eos(), dictionary.unk())
        ...
        return cls(args, dictionary, output_dictionary, targets=targets, ngram_generation_model=ngram_generation_model, ngram_warmup_updates=args.ngram_warmup_updates, ngram_dist_size=args.ngram_dist_size, eval_ngram_min_ctxlen=args.eval_ngram_min_ctxlen, eval_ngram_min_accum_prob=args.eval_ngram_min_accum_prob, input_with_ctxlen=args.input_with_ctxlen, ngram_ctxlen_p=args.ngram_ctxlen_p, data2clst_map_path=args.data2clst_map_path, train_with_limited_ctxlen=args.train_with_limited_ctxlen, ngram_min_dist_prob=args.ngram_min_dist_prob, weighted_ctxlen=args.weighted_ctxlen)
    ...
    def _prepare_ngram(self, sample, is_training):
        ...
        sample["ngram_alpha"] = ngram_dist.new_ones(1) * 0.1
        if not is_training:
