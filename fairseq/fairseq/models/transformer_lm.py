...
class TransformerLanguageModelConfig(FairseqDataclass):
    ...
    ngram_pointer: Optional[int] = field(
        default=0, metadata={"help": "force the model to learn information out of ngram context"}
    )
    ngram_gate: bool = field(
        default=False,
        metadata={"help": "control the flow of ngram information"},
    )
...
class TransformerLanguageModel(FairseqLanguageModel):
    ...
    def build_model(cls, args, task):
        ...
        if hasattr(task, "ngram_context_len") and hasattr(task, "input_with_ctxlen"):
            if task.input_with_ctxlen:
                args.ngram_pointer = task.ngram_context_len
...
@register_model_architecture("transformer_lm", "transformer_lm_wiki103_with_ngram")
@register_model_architecture("transformer_lm", "transformer_lm_baevski_wiki103_with_ngram")
def transformer_lm_baevski_wiki103(args):
    args.decoder_layers = safe_getattr(args, "decoder_layers", 16)
    args.decoder_attention_heads = safe_getattr(args, "decoder_attention_heads", 8)
    args.dropout = safe_getattr(args, "dropout", 0.3)
    ...
    args.no_decoder_final_norm = safe_getattr(args, "no_decoder_final_norm", True)
    args.tie_adaptive_proj = safe_getattr(args, "tie_adaptive_proj", True)
    args.ngram_pointer = safe_getattr(args, "ngram_pointer", 4)
    transformer_lm_big(args)
