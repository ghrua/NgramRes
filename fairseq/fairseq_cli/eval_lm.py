...
def eval_lm(
    ...
    for sample in batch_iterator:
        ...
        if "ngram_alpha" in sample:
            if ngram_alpha <= 0:
                sample.pop("ngram_word", None)
                sample.pop("ngram_dist", None)
        ...
        for i, hypos_i in enumerate(hypos):
            ...
            ctxlen = hypo["ctxlen"]
            gd_match = hypo["gd_match"]
            src_tokens = hypo["src_tokens"]
            if remove_bos_token:
            ...
            if output_word_probs or output_word_stats:
                ...
                if output_word_probs:
                    if prob_save_path:
                        prob_data.append((
                            sample_id.item(), tuple([(x[0], x[1]) for x in word_prob]),
                            gd_match.tolist() if gd_match is not None else None,
                            ctxlen.tolist() if ctxlen is not None else None,
                            src_tokens.tolist()
                        ))
                    else:
                        logger.info(
    ...
    if prob_save_path:
        torch.save(prob_data, prob_save_path)
    avg_nll_loss = (
...
def main(cfg: DictConfig, **unused_kwargs):
    ...
    results = eval_lm(
        ...
        prob_save_path=cfg.eval_lm.prob_save_path
    )
