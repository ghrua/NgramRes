...
MODEL=$DEST/results/multitask.xfm.base.drop0.3.wmt19.bpe.fp16.5gram.$alpha/checkpoint_best.pt

for DOMAIN in it koran law medical subtitles
do
    CUDA_VISIBLE_DEVICES=$gpuid nohup python eval_lm.py \
    $DEST/data-bin/multi-task-lm-en-$DOMAIN-bin \
        --gen-subset $SUB \
        --task language_modeling_with_ngram \
        --path $MODEL \
        --distributed-world-size 1 \
        ...
        --ngram-generation-model-cache "${DEST}/KNQuery/cache_5gram_multitask_${DOMAIN}/train.en.tok.bpe32k.5gram.arpa.cache" \
        --ngram-module-path $DEST/KNQuery/ > $MODEL.$SUB.$DOMAIN.$alpha.log &
    let "gpuid=gpuid+1"
done
